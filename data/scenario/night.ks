...
function getPC(c){return calms[c-1]+getEl(f.like,pairIdx(wolfChar,c));}
function getLiar(b){return getEl(f.liar,pairIdx(wolfChar,b));}
...
if(!target&&coCount>=3){
var cands=[];
for(var c=1;c<=5;c++){if(c===wolfChar||!isAlive(c)||isCO(c))continue;cands.push(c);}
